...
{
    public async Task<UserResponse> ExecuteAsync(Guid userIdentifier, CancellationToken cancellationToken)
    {
        var user = await repository.GetUserByIdentifierAsync(userIdentifier, cancellationToken)
